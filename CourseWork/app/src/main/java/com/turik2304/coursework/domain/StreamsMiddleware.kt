...
class StreamsMiddleware(private val needAllStreams: Boolean) : Middleware<GeneralActions, GeneralUiState> {
    ...
    override fun bind(actions: Observable<GeneralActions>, state: Observable<GeneralUiState>): Observable<GeneralActions> {
        return actions.ofType(GeneralActions.LoadItems::class.java)
            .flatMap {
                return@flatMap ZulipRepository.getStreams(needAllStreams = needAllStreams).toViewTypedItems()
